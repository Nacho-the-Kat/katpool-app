...
import { getUserInfo, getAccessToken, refreshJWT, verifyJWT, encryptToken, createJWT, decryptToken, UPHOLD_BASE_URL, type AccessTokenResponse } from "./authorizationCodeFlow";
...
async function refreshAccessToken(userId: string): Promise<string | null> {
  try {
    ...
    if (!refreshResponse.ok) {
      monitoring.error(`uphold: Failed to refresh token for user ${userId}: ${refreshResponse.status}`);
      return null;
    }
    ...
    monitoring.error(`uphold: Error refreshing access token for user ${userId}: ${JSON.stringify(error)}`);
    return null;
  }
}

// Helper function to make authenticated API calls with automatic token refresh
export async function makeAuthenticatedUpholdRequest(userId: string, endpoint: string, options: RequestInit = {}) {
  try {
    const userInfo = await db.getUserDetails(userId);
    if (!userInfo) {
      return null;
...
router.post('/api/users/payment-preferences', async (req: Request, res: Response): Promise<any> => {
  try {
    ...
    monitoring.log(`uphold: Payment preferences saved for user ${userId}: Network=${payoutNetwork}, Asset=${payoutAsset}`);
    return res.status(200).json({ 
