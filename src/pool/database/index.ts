...
export default class Database {
  ...
  async addOrUpdateUserPayoutPreference(user_id: string, asset_code: string, network: string)
  : Promise<boolean> {
    const client = await this.pool.connect();
    try {
      await client.query(
  ...
  async getUserDetails(uphold_id: string) {
    const client = await this.pool.connect();
    try {
      const res = await client.query(`SELECT * FROM uphold_connections WHERE uphold_id = $1`, [uphold_id]);
